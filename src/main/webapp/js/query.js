...
const PROPOSAL_FILTERS = ["Proposal Submitted","Include Dates"];
...
const PROPOSAL_SE_AND_NE_KEYS = new Array("warehouse", "item","scope", "region", "status", "permitApp", "initiated","proposalSubmitted","asBuilts","salvageValue","alarmHvacForm", "zachNotes");
const ACTIVE_SE_AND_NE_KEYS = new Array("warehouse", "item","scope","region", "status", "permitApp", "scheduledStartDate","scheduledTurnover","asBuilts","salvageValue","alarmHvacForm", "zachNotes");
...
const INVOICE_KEYS = new Array("mcsNum","warehouse", "item","class","scheduledStartDate","scheduledTurnover","invoiced", "shouldInvoice", "projectNotes");
...
const SUBMITED_PROPOSALS_KEY = new Array("stage", "warehouse", "item","scope","class","manager", "supervisor", "region", "status","initiated","siteSurvey","costcoDueDate","proposalSubmitted",
		"type","projectNotes");
...
//Field for Permit Status report
const ACTIVE_PERMIT_STATUS = new Array();
...
//populate the parameters of the reportTypePicker. Drop down
...
function reportCreator(elem)
{
	switch(elem.value)
	{
		...
		case 'Active':
			console.log($('.reportTypePicker :selected').val()+'_A');
		...
		case 'ProposalsSubmited':
			generateReport($('.reportTypePicker :selected').val()+'_PS');
			break;
		case 'Inactive':
...
{
	//gets all the fields for the report type
	...
	switch(reportType){
		...
		case ACTIVE_REPAIR:
			...
			//NEED TO EDITS TO THE SPECIFICS OF ANDY
		case ACTIVE_HVAC:
			stage.push(ACTIVE_STAGE);
		...
		case CLOSED_RX:
			stage.push(CLOSED_STAGE);
			title = "Closed RX Projects";
			pType.push(PROJECT_TYPE_RX);
			break;
		case BUDGETARY_RX:
			stage.push(BUDGETARY_STAGE);
			title = "Budgetart RX Projects";
			pType.push(PROJECT_TYPE_RX);
			break;
...
function generateOptions(key)
{
	...
	if(value<1)
		...
		for(var i = 0; i < PROPOSAL_FILTERS.length; i++)
			{
			div =  document.createElement('div');
		 document.body.appendChild(div);
		 //div.innerHTML = '<button onclick="selectionFilter(' + key + ',' + PROPOSAL_FILTERS[i] + ' )">' + PROPOSAL_FILTERS[i]  + '</button>';
		//div.innerHTML = '<button value = "Proposals"  id = ' + PROPOSAL_FILTERS[i] +  '  onclick="selectionFilter(id)"  >' + PROPOSAL_FILTERS[i]  + '</button>';
		div.innerHTML = '<button value = "ProposalSubmitted" name = "Proposals"  id = "proposalSubmitted"  onclick="selectionFilter(id)"  >' + PROPOSAL_FILTERS[i]  + '</button>';
